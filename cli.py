...
from db import (
    ...
    setup_logging,
    supabase_client,
    upsert_playlist_stats,  # Move this import to db
)
from worker.worker import (  # Changed from main to worker.worker
    fetch_pending_jobs,
    handle_job,
    mark_job_status,
    worker_loop,
...
def run(poll_interval, batch_size, max_runtime, backend, dry_run):
    ...
    if dry_run:
        ...
        logger.warning("Running in DRY-RUN mode: No DB writes will occur.")

    async def _run():
        init_supabase()
        # Inject correct backend into service
        global yt_service
...
def process(playlist_url, backend, dry_run):
    ...
    async def _process():
        ...
        job = {
            "id": int(datetime.utcnow().timestamp()),  # fake id for local run
            "playlist_url": playlist_url,
            "status": "pending",  # Add initial status
        }

        try:
            # Properly await status updates
...
# --- Monkey patch mark_job_status + upsert for dry-run ---
async def _dry_run_mark_job_status(job_id, status, meta=None):
    """Async version of dry run status marker."""
...
def _dry_run_upsert_playlist_stats(stats):
    """Safe version of dry run stats upserter."""
...
if os.getenv("VV_DRY_RUN") == "1":
    logger.warning("Dry-run mode active at import time.")
    mark_job_status = _dry_run_mark_job_status  # Use async version
